...
const COLS = `id, code, title, slug, status, urgency, priority, attributes,
              ...
              date_first_raised, mantis_submission_date, mantis_id, mantis_reporter, mantis_assignment,
              stage, phase, phase_progress, project_mandate_date, project_mandate_comment,
              planview_item_name, planview_id, planview_type, planview_comment, planview_activity,
...
exports.get = async (req, res, next) => {
  try {
    ...
    const data = serialize('project', rows[0]);
    data.data.attributes.meetings = meetings;
    res.json(data);
  } catch (e) { next(e); }
};
...
exports.update = async (req, res, next) => {
  try {
    ...
    const fields = ['code', 'title', 'slug', 'status', 'urgency', 'priority', 'attributes',
                    ...
                    'stage', 'phase', 'phase_progress', 'project_mandate_date', 'project_mandate_comment',
                    'planview_item_name', 'planview_id', 'planview_type', 'planview_comment', 'planview_activity'];
    const JSONB_FIELDS = new Set(['attributes', 'highlights', 'concerns', 'next_steps']);
    const sets = [];
    const vals = [];
    let i = 1;
    for (const f of fields) {
      if (a[f] !== undefined) {
        sets.push(`${f} = $${i++}`);
        vals.push(JSONB_FIELDS.has(f) ? JSON.stringify(a[f]) : a[f]);
      }
    }
