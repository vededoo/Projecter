...
function _serialize(row, extras = {}) {
  return {
    ...
    attributes: {
      ...
      contact_name:            extras.contact_name || null,
      suggested_contact_id:    row.suggested_contact_id,
      suggested_contact_name:  extras.suggested_contact_name || null,
      ...
      first_segment_start:     extras.first_segment_start ?? null,
...
/** Résout les noms de contacts pour une liste de IDs. Retourne Map<id, name>. */
async function _resolveContactNames(ids) {
  const unique = [...new Set(ids.filter(Boolean))];
  if (!unique.length) return new Map();
  const { rows } = await query(
    `SELECT id, first_name, last_name FROM contacts WHERE id = ANY($1::int[])`,
    [unique]
  );
  return new Map(rows.map(r => [r.id, [r.first_name, r.last_name].filter(Boolean).join(' ')]));
}

/** Premier start (en secondes) de chaque speaker dans les segments. */
function _firstSegmentByLabel(segments) {
  const first = new Map();
  for (const seg of segments || []) {
    if (seg.speaker && !first.has(seg.speaker)) first.set(seg.speaker, seg.start ?? 0);
  }
  return first; // Map<label, startSec>
}
...
exports.list = async (req, res, next) => {
  try {
    ...
    // Enrichir avec les stats et les noms de contacts
    const stats      = _computeSpeakerStats(meeting.transcription_segments);
    const firstSegs  = _firstSegmentByLabel(meeting.transcription_segments);
    const totalDuration = [...stats.values()].reduce((s, v) => s + v, 0);

    const contactIds = rows.flatMap(r => [r.contact_id, r.suggested_contact_id]);
    const nameMap    = await _resolveContactNames(contactIds);

    const data = rows.map((row) => {
      const s = _serialize(row, {
        contact_name:           row.contact_id           ? nameMap.get(row.contact_id)           || null : null,
        suggested_contact_name: row.suggested_contact_id ? nameMap.get(row.suggested_contact_id) || null : null,
        first_segment_start:    firstSegs.get(row.label) ?? null,
      });
...
exports.sync = async (req, res, next) => {
  try {
    ...
    const firstSegs      = _firstSegmentByLabel(segments);
    const totalDuration  = [...stats.values()].reduce((s, v) => s + v, 0);

    const inserted = [];
    for (const [label, labelDuration] of stats.entries()) {
      const { rows } = await query(
    ...
    const data = inserted.map(row => {
      const s = _serialize(row, { first_segment_start: firstSegs.get(row.label) ?? null });
      s.attributes.total_duration_s = stats.get(row.label) || row.total_duration_s || 0;
      s.attributes.total_pct = totalDuration > 0
        ? Math.round(((s.attributes.total_duration_s / totalDuration) * 100) * 10) / 10
        : 0;
...
exports.identify = async (req, res, next) => {
  try {
    ...
    // Résoudre les noms pour la réponse
    const suggestedIds = updated.map(r => r.suggested_contact_id).filter(Boolean);
    const nameMap      = await _resolveContactNames(suggestedIds);
    res.json({ data: updated.map(r => _serialize(r, { suggested_contact_name: nameMap.get(r.suggested_contact_id) || null })) });
  } catch (e) {
...
exports.update = async (req, res, next) => {
  try {
    ...
    const nameMap = await _resolveContactNames([rows[0].contact_id, rows[0].suggested_contact_id]);
    res.json({ data: _serialize(rows[0], {
      contact_name:           rows[0].contact_id           ? nameMap.get(rows[0].contact_id)           || null : null,
      suggested_contact_name: rows[0].suggested_contact_id ? nameMap.get(rows[0].suggested_contact_id) || null : null,
    }) });
  } catch (e) { next(e); }
};
