...
function pipeProgress(textId, clientReq, clientRes) {
  ...
  const upstreamReq = transport.get(
    {
      ...
      path:     parsedUrl.pathname,
      // Pas de timeout socket : le stream SSE peut durer plusieurs heures
      timeout:  0,
    },
    (upstreamRes) => {
  ...
  // Keep-alive socket pour maintenir la connexion pendant les longues transcriptions
  upstreamReq.on('socket', (socket) => {
    socket.setKeepAlive(true, 30_000); // keepalive toutes les 30s
    socket.setTimeout(0);              // désactiver le timeout socket
  });
