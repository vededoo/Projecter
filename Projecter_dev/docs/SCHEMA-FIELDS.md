...
Trois champs texte/JSONB cohabitent sur `meetings` — rôles distincts :

| Champ            | Type    | Rôle                                                                                                                                                                                                                                  |
| ---------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `raw_transcript` | `text`  | Source brute **immuable** — transcript brut collé ou généré par Whisper. Écrit une seule fois par `ingest_meeting_extraction`. Ne jamais modifier après ingestion.                                                                    |
| `minutes`        | `text`  | CR rédigé **lisible** — texte structuré destiné à être partagé (sections, titres, mise en forme markdown). Optionnel, écrit par PATCH ou IA.                                                                                          |
| `ai_report`      | `jsonb` | Rapport structuré **machine** produit par le LLM. Libre, mais convention attendue : `{ executive_summary, topics[], decisions[], actions[], attendees[], generated_at }`. Distinct du créneau `executive_summary` (colonne scalaire). |

`meeting_topics` : `position`, `title`, `summary`, `type` (topic_type), `commitment_level`, `axes` (project_axis[]), `synthesis`, `confidence_level`, `project_topic_id` (FK project_topics).

`meeting_decisions` : `description`, `impact`, `position`, + champs DACI (migration 005).

`meeting_actions` : `description`, `owner_id` (FK contacts), `owner_raw`, `deadline`, `status` (action_status), `notes`.

`meeting_attendees` : `contact_id` (FK contacts), `status` (attendance_status), `role`.
