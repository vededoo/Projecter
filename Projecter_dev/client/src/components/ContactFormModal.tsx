...
const EMPTY: ContactFormValues = {
  ...
  office_location: null, street_address: null, postal_code: null,
  city: null, state: null, country: null,
  floor: null, office_ref: null,
  preferred_language: null, usage_location: null,
...
export function ContactFormModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r: any = await api.get('/roles?active=true');
        if (!cancelled) {
          setRoles((r.data || [])
            .filter((x: any) => x.attributes.active)
            .map((x: any) => ({ id: Number(x.id), label: x.attributes.label })));
        }
    ...
    >
      <form onSubmit={onSubmit} autoComplete="off">
        <div className="grid-2">
          ...
          <div>
            <label>Default project role</label>
            <select
              value={values.default_role_id ?? ''}
              onChange={(e) => set('default_role_id', e.target.value ? parseInt(e.target.value, 10) : null)}
            >
          ...
          <div>
            <label>Floor</label>
            <input value={values.floor || ''} onChange={(e) => set('floor', e.target.value || null)} />
          </div>
          <div>
            <label>Office ref</label>
            <input value={values.office_ref || ''} onChange={(e) => set('office_ref', e.target.value || null)} />
          </div>

          <div style={{ gridColumn: '1 / -1' }}>
            <label>Street address</label>
            <input value={values.street_address || ''} onChange={(e) => set('street_address', e.target.value || null)} />
          </div>

          <div>
            <label>Postal code</label>
            <input value={values.postal_code || ''} onChange={(e) => set('postal_code', e.target.value || null)} />
          </div>
          <div>
            <label>State / Region</label>
            <input value={values.state || ''} onChange={(e) => set('state', e.target.value || null)} />
          </div>

          <div>
            <label>Preferred language</label>
            <input value={values.preferred_language || ''} onChange={(e) => set('preferred_language', e.target.value || null)} />
          </div>
          <div>
            <label>Usage location</label>
            <input value={values.usage_location || ''} onChange={(e) => set('usage_location', e.target.value || null)} />
          </div>

          {(initial as any)?.org_unit_path && (
            <div style={{ gridColumn: '1 / -1' }}>
              <label>Org unit (via membership)</label>
              <div style={{
