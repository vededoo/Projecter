...
type ColKey =
  ...
  | 'manager_name' | 'office_location' | 'street_address' | 'postal_code'
  | 'city' | 'state' | 'country'
  | 'floor' | 'office_ref' | 'org_unit_label' | 'org_unit_path'
  | 'preferred_language' | 'usage_location'
  | 'employee_id' | 'sam_account_name' | 'graph_synced_at' | 'updated_at';

// Type de filtre proposé sous l'en-tête de colonne
type FilterKind = 'none' | 'text' | 'org';
...
interface ColumnDef {
  ...
  width?: string;
  // Filtre par colonne : 'text' = input ILIKE, 'org' = select organisation, 'none' = non filtrable
  filter: FilterKind;
  // Nom du paramètre serveur (par défaut = key). Ex: organization_code → 'organization'
  filterParam?: string;
...
const COLUMNS: ColumnDef[] = [
  { key: '#',                 label: '#',          sortable: false, defaultVisible: true,  width: '48px', filter: 'none',
    render: (_, idx) => <span className="muted">{idx + 1}</span> },
  { key: 'id',                label: 'ID',         sortable: true,  defaultVisible: true,  width: '60px', filter: 'text',
    render: (c) => <span className="muted" style={{ fontFamily: 'monospace', fontSize: 12 }}>#{c.id}</span> },
  { key: 'last_name',         label: 'Last name',  sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => <strong>{c.last_name}</strong> },
  { key: 'first_name',        label: 'First name', sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => c.first_name || '—' },
  { key: 'email',             label: 'Email',      sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => c.email ? <a href={`mailto:${c.email}`}>{c.email}</a> : <span className="muted">—</span> },
  { key: 'phone',             label: 'Phone',      sortable: false, defaultVisible: false, filter: 'text',
    render: (c) => c.phone || <span className="muted">—</span> },
  { key: 'job_title',         label: 'Job title',  sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => c.job_title || <span className="muted">—</span> },
  { key: 'department',        label: 'Department', sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => c.department ? <span className="badge">{c.department}</span> : <span className="muted">—</span> },
  { key: 'company_name',      label: 'Company',    sortable: true,  defaultVisible: false, filter: 'text',
    render: (c) => c.company_name || <span className="muted">—</span> },
  { key: 'organization_code', label: 'Org',        sortable: true,  defaultVisible: true,  filter: 'org', filterParam: 'organization',
    render: (c) => c.organization_code ? <span className="badge">{c.organization_code}</span> : <span className="muted">—</span> },
  { key: 'org_unit_path',     label: 'Org unit',   sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => c.org_unit_path
      ? <span className="badge" title={c.org_unit_path} style={{ fontFamily: 'monospace', fontSize: 11 }}>{c.org_unit_path}</span>
      : <span className="muted">—</span> },
  { key: 'manager_name',      label: 'Manager',    sortable: true,  defaultVisible: true,  filter: 'text',
    render: (c) => c.manager_name || <span className="muted">—</span> },
  { key: 'office_location',   label: 'Office',     sortable: false, defaultVisible: false, filter: 'text',
    render: (c) => c.office_location || <span className="muted">—</span> },
  { key: 'floor',             label: 'Floor',      sortable: true,  defaultVisible: false, filter: 'text', width: '80px',
    render: (c) => c.floor || <span className="muted">—</span> },
  { key: 'office_ref',        label: 'Office ref', sortable: true,  defaultVisible: false, filter: 'text', width: '90px',
    render: (c) => c.office_ref || <span className="muted">—</span> },
  { key: 'street_address',    label: 'Address',    sortable: false, defaultVisible: false, filter: 'text',
    render: (c) => c.street_address || <span className="muted">—</span> },
  { key: 'postal_code',       label: 'Postal',     sortable: true,  defaultVisible: false, filter: 'text', width: '90px',
    render: (c) => c.postal_code || <span className="muted">—</span> },
  { key: 'city',              label: 'City',       sortable: true,  defaultVisible: false, filter: 'text',
    render: (c) => c.city || <span className="muted">—</span> },
  { key: 'state',             label: 'State',      sortable: true,  defaultVisible: false, filter: 'text',
    render: (c) => c.state || <span className="muted">—</span> },
  { key: 'country',           label: 'Country',    sortable: true,  defaultVisible: false, filter: 'text',
    render: (c) => c.country || <span className="muted">—</span> },
  { key: 'preferred_language', label: 'Lang',      sortable: true,  defaultVisible: false, filter: 'text', width: '80px',
    render: (c) => c.preferred_language || <span className="muted">—</span> },
  { key: 'usage_location',    label: 'Usage loc.', sortable: true,  defaultVisible: false, filter: 'text', width: '90px',
    render: (c) => c.usage_location || <span className="muted">—</span> },
  { key: 'employee_id',       label: 'Emp. ID',    sortable: true,  defaultVisible: false, filter: 'text',
    render: (c) => c.employee_id || <span className="muted">—</span> },
  { key: 'sam_account_name',  label: 'SAM',        sortable: false, defaultVisible: false, filter: 'text',
    render: (c) => c.sam_account_name || <span className="muted">—</span> },
  { key: 'graph_synced_at',   label: 'Graph sync', sortable: true,  defaultVisible: false, filter: 'none',
    render: (c) => fmtDate(c.graph_synced_at) },
  { key: 'updated_at',        label: 'Updated',    sortable: true,  defaultVisible: false, filter: 'none',
...
export function ContactsPage() {
  ...
  // Filtres par colonne : { [colKey]: valeur } — affichés sous chaque en-tête
  const [colFilters, setColFilters] = useState<Record<string, string>>({});
  const debColFilters = useDebounced(colFilters, 350);
  ...
  useEffect(() => { setPage(1); }, [debQ, debColFilters, activeFlag, pageSize]);
  ...
  useEffect(() => {
    ...
    // Filtres par colonne → param serveur (filterParam si défini, sinon la clé)
    for (const [key, val] of Object.entries(debColFilters)) {
      if (!val || !val.trim()) continue;
      const def = COLUMNS.find((c) => c.key === key);
      params.set(def?.filterParam || key, val.trim());
    }
  ...
  }, [debQ, debColFilters, activeFlag, sortCol, sortDir, page, pageSize, refreshKey]);
  ...
  const setColFilter = (key: ColKey, val: string) =>
    setColFilters((s) => {
      const next = { ...s };
  ...
  const resetFilters = () => {
    setQ(''); setColFilters({}); setActiveFlag('true');
  };
  ...
  return (
    <div>
      ...
      {/* ─── Barre de recherche globale (compacte) ──────────────────── */}
      <div className="card" style={{ display: 'flex', gap: 8, alignItems: 'end', flexWrap: 'wrap' }}>
        ...
        <div>
          <button className="btn" onClick={resetFilters} title="Reset search + all column filters">
            Reset{activeFilterCount > 0 ? ` (${activeFilterCount})` : ''}
          </button>
        </div>
        <div className="muted" style={{ fontSize: 11, flexBasis: '100%' }}>
          Tip: filter each column using the input row right under the column titles.
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflowX: 'auto' }}>
        <table>
          <thead>
            ...
            {/* Ligne de filtres : un filtre par colonne affichée, sous son titre */}
            <tr className="filter-row">
              {visibleCols.map((col) => (
                <th key={`f-${col.key}`} style={{ padding: '4px 6px' }}>
                  {col.filter === 'text' && (
                    <input
                      value={colFilters[col.key] || ''}
                      onChange={(e) => setColFilter(col.key, e.target.value)}
                      placeholder="Filter…"
                      style={{ width: '100%', fontSize: 12, padding: '3px 6px', fontWeight: 'normal' }}
                    />
                  )}
                  {col.filter === 'org' && (
                    <select
