...
export function RisksPage() {
  ...
  const sortedRisks = useMemo(() => [...risks].sort((a, b) => {
    let av: any, bv: any;
    const attr = (x: RiskItem) => x.attributes as any;
    if (sortCol === 'probability' || sortCol === 'impact' || sortCol === 'severity') {
      av = LEVEL_ORDER[attr(a)[sortCol] || ''] || 0;
      bv = LEVEL_ORDER[attr(b)[sortCol] || ''] || 0;
    } else if (sortCol === 'due_date') {
      av = attr(a).due_date || '';
      bv = attr(b).due_date || '';
    } else {
      av = (attr(a)[sortCol] || '').toLowerCase();
      bv = (attr(b)[sortCol] || '').toLowerCase();
    }
  ...
  return (
    <div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <table>
          <thead><tr>
            <th>Projects</th><SortTh col="label" label="Label" /><SortTh col="probability" label="Prob" /><SortTh col="impact" label="Impact" />
            <SortTh col="severity" label="Severity" /><SortTh col="status" label="Status" /><SortTh col="due_date" label="Due" /><th style={{ width: 120 }}></th>
          </tr></thead>
          <tbody>
            {sortedRisks.map(r => (
              <tr key={r.id}>
                ...
                <td>
                  <InlineEdit
                    value={r.attributes.label}
                    onSave={async label => {
                      await api.patch(`/risks/${r.id}`, { label }, 'risk');
                      setRisks(prev => prev.map(x => x.id === r.id ? { ...x, attributes: { ...x.attributes, label } } : x));
                    }}
                  />
                </td>
